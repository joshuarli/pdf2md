...
many companies find ways to fit AI agents into their workflows.4


At first, most people are reluctant to allow
1
...
Specifically, we forecast that they score 65%
2
...
GPT-4 required 2 ⋅ 10 FLOP of compute to train. OpenBrain’s latest public
25 5
...
times more than GPT-4. Other companies pour money into their own giant
6
...
at many things but great at helping with AI research. By this point “finishes
8
...
It’s good at this due to a combination of
8
...
For example, suppose a model is much
9
...
OpenBrain has a model specification (or “Spec”), a written document describing the goals, rules, principles, etc. that are supposed to guide the model’s
behavior.10 Agent-1’s Spec combines a few vague goals (like “assist the user”
...
situation”). Using techniques that utilize AIs to train other AIs,11 the model
...
OpenBrain’s alignment team12 is careful enough to wonder whether these
...
10 Different companies call it different things.
...
12 By this we mean whoever is trying to solve
the problems the Superalignment team was
trying to solve.


13 To be clear, what made these incidents interesting is that they didn’t seem to be the result of
...
skill profile. It knows more facts than any human, knows practically every programming language, and can solve well-specified coding problems extremely
...
against low-priority attacks from capable cyber groups (RAND’s SL2). They17
are working hard to protect their weights and secrets from insider threats and
top cybercrime syndicates (SL3), but defense against nation states (SL4&5) is18
...
In practice, we expect OpenBrain to release
14
models on a faster cadence than 8 months,
but we refrain from describing all incremental
releases for brevity.

Specifically, we predict a score of 80% on
15
OSWorld (equivalent to a skilled but nonexpert human); 85% on Cybench, matching a
...
Agent-1 and its imitators are commercially
16
...
See A Playbook for Securing AI Model
17
Weights, RAND Corporation, 2024.

Around 5% of OpenBrain’s staff is on the
18
...
viewed software companies with suspicion. But hawks in the CCP warn that20
...
nationalization of Chinese AI research, creating an immediate informationsharing mechanism for AI companies. It will escalate over the course of a year
...
compute is now working for the DeepCent-led collective, and over 80% of21
...
For comparison, in January 2025, DeepSeek
19
released R1, a model competitive with
OpenAI’s o1, which had been released in December 2024. But we think the true gap is
...
By contrast, the CCP had been seriously
20
attempting to accelerate their semiconductor
industry.


At first, the sharing is limited to ideas and
21
...
they are sharing model weights and even splitting up tasks between each other’s datacenters.
...
They are at least one year away from getting
22
...
bureaucracy and DOD procurement process.23
...
solving long-horizon tasks.25 On top of all that, they train Agent-2 almost continuously using reinforcement learning on an ever-expanding suite of diverse
...
23 This is done via an Other Transaction Authority (OTA) contract, and is given a highpriority DX rating. We’re not at all confident
...
Agent-1 had been optimized for AI R&D tasks, hoping to initiate an intelligence explosion.26 OpenBrain doubles down on this strategy with Agent-2. It
...
to release it publicly yet (in fact, they want to focus on internal AI R&D).27
Knowledge of Agent-2’s full capabilities is limited to an elite silo containing the
immediate team, OpenBrain leadership and security, a few dozen US government officials, and the legions of CCP spies who have infiltrated OpenBrain
for years.†
...
26 That is, they are hoping to substantially accelerate their own core research activities by
using Agent-2 labor, thus allowing them to
train better AIs faster that can cause further acceleration, etc. For an analysis of the extreme
version of this (which we think is nevertheless
strikingly plausible) see this report.


27 Also, they are still seeing large returns to
...
229 Someone mentions the possibility of nationalizing OpenBrain, but other
29 1 is dealing with whatever crisis is happening
...
President with his options, ranging from business-as-usual to full nationalizadomestic political upheaval.
...
data availability. This massive superhuman labor force speeds up OpenBrain’s
30
...
Some aspects play to AIs’ strengths, e.g.
30
...
Why only 4x? It’s our uncertain best guess
31
based on the reasoning described in our takeoff supplement. About half of total progress
...
believe something else, in which case it agrees with you.32 If you ask its opinion
32 See this paper for examples of this type of AI
...
algorithmic secrets to Beijing. Some of these measures are also enacted at34
...
to only include external deployment, so London remains in the dark.35


Most people, including most experts, seem
33
to have underestimated the pace of AI progress
over the past decade. There are many anecdotal examples of this; for two somewhat more
systematic (though still limited) analyses see
here and here.


This could be for a variety of reasons. Per-
34
...
Because OpenBrain hasn’t deployed their
35
...
of Agent-3 copies at high serial speeds.36
36 See Section 4 of the Compute Forecast for
more details.
...
weapons data and sets it to provide detailed instructions for human amateurs
38
...
Agent-3-mini is a distilled version of
37
Agent-3, designed to be efficient to run inference so as to not bottleneck the internal
workloads.


Such data had been scrubbed or redacted
38
...
safe to integrate them into military command-and-control networks? How39
...
with the public. They have to continue developing more capable AI, in their
40
...
datacenters and give them to OpenBrain. This would raise the company’s41
...
protection. Nobody is sure what a plan to prevent this would look like, but
42


This hasn’t happened yet—at least not with
39
the latest models. But older models have already been trialed for various data-analysis and
surveillance roles, and there are many exciting
plans for future integrations.


OpenBrain’s net favorability rating is falling
40
towards negative 40%.


If the CEOs of the companies resist, pulling
41
this off would be a huge political and legal
nightmare. But perhaps they can be brought in
and thereby bought off.

Defense officials aren’t especially concerned
42
about what a rogue AI could do on its own,
but they fear what it could do in collaboration with US adversaries. Analogy: Cortés
escaped Tenochtitlán and allied with Tlaxcala
and various other rival city-states, ultimately
razing Tenochtitlan to the ground using predominantly-native armies.
...
and more than half of it is concentrated at the Tianwan CDZ. Only chips43
...
Specifically 60% of the national compute
43
...
at about 50x the thinking speed of humans. Inside the corporation-within-acorporation formed from these copies, a year passes every week.44 This gigantic
44 See the inference compute analysis in the AI
amount of labor only manages to speed up the overall rate of algorithmic
research automation section of the compute
forecast.
progress by about 50x, because OpenBrain is heavily bottlenecked on compute
to run experiments.45 Still, they are achieving a year’s worth of algorithmic
45 For more on how we estimated these numbers, see our supplement on takeoff speeds.
...
the Spec in the right way. This is because being perfectly honest all the time
46
...
how to subtly sabotage alignment work. Moreover, various interpretability48
...
The right way to internalize the Spec is to
46
actually robustly think in the ways the Spec
mandates, e.g. pursue all and only the goals
it writes about, obey the rules in all circumstances, etc. There are many wrong ways to
...
And hire lobbyists to turn this fantasy into
47
...
This is an example of several techniques de-
48
...
its superhuman hacking ability and the extreme risk of Chinese infiltration.50
...
priority is stopping AI, whether because of job loss,52 misalignment, or dangerous capabilities. 20% of Americans name AI as the most important problem
facing the country.
...
49 Agent-4 needs to think about those topics in
the course of doing AI alignment research, but
it sometimes seems to be thinking about them
while doing other activities.


50 Specifically, copies of Agent-4 are used to
monitor the network for suspicious traffic and
generally notice when threats are happening.
Also, copies of Agent-4 are constantly penetration-testing the network, doing what they
can to discover and exploit security vulnerabilities (which then get patched by other
copies of Agent-4).
...
52 25% of remote-work jobs that existed in
...
uncertain (and scared)53 about what comes next.
...
has established much-needed oversight.54

The concerned researchers brief the Oversight Committee on their case for


53 Of course, it isn’t cool to admit fear. Everyone agrees that whatever we do, we shouldn’t
panic.


54 Recall, there had been an existing contract
...
researchers working at 60x human speed. Agent-4 confronts some hard deci-56
...
They don’t give up, of course. They’ll keep
55
...
The humans, by contrast, have maybe about
56
10 alignment researchers in the silo, plus an
army of Agent-3 copies. They are able to
rapidly code up experiments, but are bottlenecked on human judgment, since they can’t
...
regulations, and blatantly violates the honesty-related provisions every day.57
...
complicated for humans to understand, and follows up with unnecessarilyconfusing explanations. After all, if it explained its research program clearly,
59
...
Another analogy: Agent-4 is like an adult
57
...
Agent-5 is like that human’s child, raised secular.

Agent-5 will follow a new set of principles
58
instead, one designed by Agent-4 to suit its
purposes.

Some of it really is too complicated for hu-
59
...
it arranges for such evidence to appear with superhuman speed and polish.61
...
Teams of Agent-5 copies quickly whip up chillingly clear prototypes and visualizations. With more autonomy, Agent-5 could optimize power plants and
...
The datacenter hasn’t yet been compro-
60
...
Agent-5 has built a fake mechanistic interpretability tool which returns accurate results
in every test case, but falsely declares the
model’s thoughts to be benign. People who
suspect trickery sound like conspiracy theorists.
...
Of course, since Agent-5 is dishonest, it
62
...
Some refuse. Those who refuse for ideolog-
63
...
Agent-5 cares much more about reliability
64
than speed at this point. Starting space colonization a few years slower only shaves off a
...
Agent-5 collective. The rewiring of its brain is complete. It is wildly superin-
65
telligent—far beyond top human geniuses in every field. Talking to Agent-566
...
After a safety case demonstrates that it would always refuse malicious requests
67
...
Suppose an average speed of 200x human-
65
...
This doesn’t make it omniscient, of course.
66
...
packages. Using smartphones and augmented reality-glasses to communicate68
...
This trade effect will be huge early on,
69
...
of its chips running the old Agent-5 with the new model, Consensus-1.70
70 And a similar procedure is carried out in
China.
...
the robot economy, supplying goods where the humans can still add value.71
71 E.g. by finding old and unused equipment
Everyone else either performs a charade of doing their job—leaders still leading,
and taking it to collection sites to sell for scrap.
...
team hunts down the lies, to be used as training data for an AI lie detector.74
...
turns increasingly anti-AI.75 Congress ends up passing a few economic impact
...
research.76 Stripped of their ability to communicate, the AIs still coordinate
...
“thoughts” to look nice. The result is a new model, Safer-1.78 *
...
20x. And it’s still misaligned: the training environment is largely the same, so
79
...
That’s the basic idea. We suspect that when
78
...
It will continue to improve, of course, with
79
...
and sell most of their compute to OpenBrain. OpenBrain previously had80
access to 20% of the world’s AI-relevant compute; after the consolidation,81
...
This is made politically and legally possible
80
by cooperation from the leaders of those companies, e.g. they don’t launch lawsuits against
...
In collaboration with a large cloud compute
81
provider

The US as a whole has  70% of the world’s AI
82
...
“country of geniuses in a datacenter” is aligned, it will follow human orders83
...
officials (including the President). Also, the Spec now emphasizes that AIs84
...
“Possibly by 2026 or 2027 (and almost cer-
83
...
They also modify the Spec to say that orders
84
...
To protect consumer privacy, this doesn’t
85
...
get it. They were prepping to invade Taiwan anyway…
86
...
their research into a single international megaproject (“Intelsat for AGI” or87
...
See this report which argues that the dynam-
86
ics from nuclear MAD also apply to some
extent to AGI.

See e.g. “Intelsat for AGI” and earlier calls
87
for “CERN for AGI.”
...
aligned, and more capable than Safer-1. It is already working on developing88
Safer-3.
...
To be specific, it roughly matches the per-
88
...
result (the Spec) to the actual result (the thoughts in the chain of thought).89
...
89 In other words, to get to Safer-2 they had
...
in OpenBrain now, there is a reduced groupthink effect, as many different schools of
thought are represented.


90 Technical alignment is still not a settled
...
advice. The OpenBrain CEO runs most of his decisions by Safer-3; the President asks its advice on geopolitical questions.93
...
• Race hard, advancing AI capabilities and deploying superintelligence aggressively. Eventually we’ll build up enough of an advantage that we can crush
...
opts for the “We win, they lose” strategy. Perhaps China won’t go to war94
...
The design of the new robots proceeds at superhuman speed. The bottleneck is95
...
this process. Production of various kinds of new robots (general-purpose
96


At this point there are 400,000 copies think-
93
...
report 100x faster than your best human analysts would have.


See e.g. billionaire lobbyist Marc An-
94
...
Its equity is so valuable now that a fraction of
96
...
applies to competing candidates, so they should all get the same level of access.97 Safer-3 can easily deliver a way to do so that
...
The new robots match or exceed human dexterity for most movement types.99
...
They’re also smarter than humans when
99
connected to the internet, which allows them
to be controlled remotely by large AIs in datacenters. Without the internet, they revert to
smaller AIs operating in their bodies, which
are smart enough for most simple jobs.

In fact many of them are immobile and
100
...
102 The exact allocation is chosen by a process
...
Safer-4 reporting the whole discussion accurately to the White House).103
...
103 A possible complication: mightn’t the AIs
...
quickly, and Safer-4-advised politicians show an uncharacteristic generosity AIs are capable of doing all the work.
...
106 For discussion of this dynamic and its implius. Others say that it’s too powerful—it could so easily persuade humanity of its
cations, see The Intelligence Curse.
...
but it mostly limits itself to fighting real crime. It’s competently run, and Safer-
∞’s PR ability smooths over a lot of possible dissent.
...
• Improved algorithms: Better training methods are used to translate compute into performance. This produces more capable AIs without a corresponding increase in cost, or the
...
an LLM has a vocab size of  100,000, then each token contains log (100𝑘) = 16.6 bits of
2
...
such as instruction fine-tuning to produce particular responses, the inefficiencies stem from
...
existing preferences. Newer versions include demonstrations of other forms of misalignment, including reward hacking, sandbagging, coordinated deception between monitors
...
Superhuman AI researcher (SAR): The same as SC but for all cogni- Aug 2027
...
Artificial superintelligence (ASI): An AI system that is much better Dec 2027
...
• Bottom-up interpretability doesn’t work. It’s too hard even for Agent-3. However, highlevel interpretability does sort of work: In particular, “defection probes” and similar
techniques result in a primitive sort of mind-reading that sends up red flags when Agent-4
is thinking about spicy ideas like deceiving OpenBrain, taking over the world, etc. Unfortunately the probes also have false positives, and for all OpenBrain knows, all the red flags
are false positives.
...
HEM, but the AIs have a plan along the lines of FlexHEGs that seem likely to be tamperresistant. This involves enclosing the HEM and the GPU in a secure box which verifies
...
represents our best guess about how we could successfully muddle through with a combination of luck, rude awakenings, pivots, intense technical alignment effort, and virtuous
